...
const init = async () => {
  ...
  const [
    ...
    createUser({ username: "frankie", password: "frankie_pw" }),
  ...
  const userFavorite = await Promise.all([
    createFavorite(frankie.id, plateSpinning.id),
    createFavorite(frankie.id, dancing.id),
    createFavorite(ethyl.id, singing.id),
    createFavorite(ethyl.id, juggling.id),
  ]);
  console.log(await fetchFavorites(frankie.id));
  await destroyFavorite(userFavorite[0].id, frankie.id);
